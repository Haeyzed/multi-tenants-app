...
type ColumnElement = HTMLDivElement;
type ColumnItemElement = HTMLButtonElement;
...
function TimePickerHour(props: TimePickerHourProps) {
  const { render, format = "numeric", className, ...hourProps } = props;
  ...
  const hoursContent = hours.map((hour) => (
...
function TimePickerMinute(props: TimePickerMinuteProps) {
  const { render, format = "2-digit", className, ...minuteProps } = props;
  ...
  const minutesContent = minutes.map((minute) => (
    <TimePickerColumnItem
      key={minute}
      value={minute}
...
function TimePickerSecond(props: TimePickerSecondProps) {
  const { render, format = "2-digit", className, ...secondProps } = props;
  ...
  const secondsContent = seconds.map((second) => (
    <TimePickerColumnItem
      key={second}
      value={second}
    ...
    props: mergeProps<"div">(
      {
        ...
        children: secondsContent,
      },
      secondProps,
    ),
    render: render || undefined,
    state: {
      slot: "time-picker-second",
    },
  });
...
function TimePickerPeriod(props: DivProps) {
  const { render, className, ...periodProps } = props;
  ...
  const periodsContent = PERIODS.map((period) => (
    <TimePickerColumnItem
      key={period}
      value={period}
    ...
    props: mergeProps<"div">(
      {
        ...
        children: periodsContent,
      },
      periodProps,
    ),
    render: render || undefined,
    state: {
      slot: "time-picker-period",
    },
  });
...
interface TimePickerSeparatorProps
  extends React.ComponentProps<"span">,
    useRender.ComponentProps<"span"> {}

function TimePickerSeparator(props: TimePickerSeparatorProps) {
  const { render, children, ...separatorProps } = props;
