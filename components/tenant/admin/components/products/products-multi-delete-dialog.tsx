...
import { useDeleteManyProducts } from "@/hooks/tenant/use-product-query"
  ...
  const handleDelete = () => {
    ...
    deleteMany.mutate(ids, {
      onSuccess: (result) => {
        toastApiSuccess(
          result.message,
          `Deleted ${ids.length} ${ids.length > 1 ? "products" : "product"}`
        )
        setValue("")
        onComplete?.()
        onOpenChange(false)
      },
      onError: (error) => toastApiError(error, "Failed to delete products"),
    })
  }
      ...
      <ResponsiveDialogContent>
        <ResponsiveDialogHeader>
          <ResponsiveDialogTitle className="flex items-center gap-2 text-destructive">
            ...
            Delete {ids.length} {ids.length > 1 ? "products" : "product"}
          </ResponsiveDialogTitle>
          <ResponsiveDialogDescription>
            Are you sure you want to delete the selected products? This action
            cannot be undone.
          </ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
