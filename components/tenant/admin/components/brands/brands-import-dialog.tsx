...
export function BrandsImportDialog({
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    importBrands.mutate(file, {
      onSuccess: (result) => {
        toastApiSuccess(result.message, "Brands imported successfully")
        onOpenChange(false)
        form.reset()
      },
      onError: (error) => toastApiError(error, "Failed to import brands"),
    })
  }
    ...
    >
      <ResponsiveDialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <ResponsiveDialogFooter>
          ...
          >
            {importBrands.isPending && <Spinner className="mr-2" />}
            Import
          </Button>
        </ResponsiveDialogFooter>
      </ResponsiveDialogContent>
