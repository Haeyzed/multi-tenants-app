...
import { useImportUnits } from "@/hooks/tenant/use-unit-query"
import { downloadUnitsImportSample } from "@/lib/services/tenant/unit-service"
...
export function UnitsImportDialog({
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    importUnits.mutate(file, {
      onSuccess: (result) => {
        toastApiSuccess(result.message, "Units imported successfully")
        onOpenChange(false)
        form.reset()
      },
      onError: (error) => toastApiError(error, "Failed to import units"),
    })
  }
    ...
    >
      <ResponsiveDialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
        <ResponsiveDialogHeader>
          ...
          <ResponsiveDialogDescription>
            Import measurement units from an Excel (.xlsx) or CSV file.
          </ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
        <form
          id="units-import-form"
        ...
        >
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await downloadUnitsImportSample("xlsx")
        ...
        <ResponsiveDialogFooter>
          ...
          >
            {importUnits.isPending && <Spinner className="mr-2" />}
            Import
          </Button>
        </ResponsiveDialogFooter>
      </ResponsiveDialogContent>
