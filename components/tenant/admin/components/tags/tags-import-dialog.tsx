...
import { useImportTags } from "@/hooks/tenant/use-tag-query"
import { downloadTagsImportSample } from "@/lib/services/tenant/tag-service"
...
export function TagsImportDialog({
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    importTags.mutate(file, {
      onSuccess: (result) => {
        toastApiSuccess(result.message, "Tags imported successfully")
        onOpenChange(false)
        form.reset()
      },
      onError: (error) => toastApiError(error, "Failed to import tags"),
    })
  }
  ...
  return (
    ...
    >
      <ResponsiveDialogContent className="max-h-[90vh] overflow-y-auto">
        <ResponsiveDialogHeader>
          <ResponsiveDialogTitle>Import Tags</ResponsiveDialogTitle>
          <ResponsiveDialogDescription>
            Import tags from an Excel (.xlsx) or CSV file.
          </ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
        <form
          id="tags-import-form"
        ...
        >
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await downloadTagsImportSample("xlsx")
        ...
        <ResponsiveDialogFooter>
          ...
          <Button
            type="submit"
            form="tags-import-form"
            disabled={importTags.isPending}
          >
            {importTags.isPending && <Spinner className="mr-2" />}
            Import
          </Button>
        </ResponsiveDialogFooter>
      </ResponsiveDialogContent>
