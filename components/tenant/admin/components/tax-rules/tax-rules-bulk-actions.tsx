...
type TaxRulesBulkActionsProps<TData> = {
  table: Table<TData>
}

export function TaxRulesBulkActions<TData>({
  table,
}: TaxRulesBulkActionsProps<TData>) {
  const { setOpen, setExportSelection, setDeleteManySelection } = useTaxRules()
  ...
  const handleExport = () => {
    const ids = selectedRows.map((row) => (row.original as TaxRule).id)
    setExportSelection({
      ids,
