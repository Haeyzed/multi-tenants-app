    ...
    importWarehouses.mutate(file, {
      onSuccess: (result) => {
        if ((result.data.failed ?? 0) > 0) {
          ...
          toastApiSuccess(result.message, "Warehouses imported successfully")
        }
        onOpenChange(false)
        form.reset()
      },
      onError: (error) => toastApiError(error, "Failed to import warehouses"),
    })
  }
    ...
    >
      <ResponsiveDialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
        <ResponsiveDialogHeader>
          ...
          <ResponsiveDialogDescription>
            Import warehouses from an Excel (.xlsx) or CSV file.
          </ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
        <form
          id="warehouses-import-form"
        ...
        >
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await downloadWarehousesImportSample("xlsx")
