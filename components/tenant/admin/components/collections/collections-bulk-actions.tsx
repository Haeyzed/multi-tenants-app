...
type CollectionsBulkActionsProps<TData> = {
  table: Table<TData>
}

export function CollectionsBulkActions<TData>({
  table,
}: CollectionsBulkActionsProps<TData>) {
  const { setOpen, setExportSelection, setDeleteManySelection } = useCollections()
  ...
  const handleExport = () => {
    const ids = selectedRows.map((row) => (row.original as Collection).id)
    setExportSelection({
      ids,
