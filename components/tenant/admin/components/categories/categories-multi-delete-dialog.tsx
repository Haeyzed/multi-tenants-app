...
import { useDeleteManyCategories } from "@/hooks/tenant/use-category-query"
  ...
  const handleDelete = () => {
    ...
    deleteMany.mutate(ids, {
      onSuccess: (result) => {
        toastApiSuccess(
          result.message,
          `Deleted ${ids.length} ${ids.length > 1 ? "categories" : "category"}`
        )
        setValue("")
        onComplete?.()
        onOpenChange(false)
      },
      onError: (error) => toastApiError(error, "Failed to delete categories"),
    })
  }
      ...
      <ResponsiveDialogContent>
        <ResponsiveDialogHeader>
          <ResponsiveDialogTitle className="flex items-center gap-2 text-destructive">
            ...
            Delete {ids.length} {ids.length > 1 ? "categories" : "category"}
          </ResponsiveDialogTitle>
          <ResponsiveDialogDescription>
            Are you sure you want to delete the selected categories? This action
            cannot be undone.
          </ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
