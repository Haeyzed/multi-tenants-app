...
export function CategoriesImportDialog({
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    importCategories.mutate(file, {
      onSuccess: (result) => {
        toastApiSuccess(result.message, "Categories imported successfully")
        onOpenChange(false)
        form.reset()
      },
      onError: (error) => toastApiError(error, "Failed to import categories"),
    })
  }
    ...
    >
      <ResponsiveDialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
        ...
        <ResponsiveDialogFooter>
          ...
          >
            {importCategories.isPending && <Spinner className="mr-2" />}
            Import
          </Button>
        </ResponsiveDialogFooter>
      </ResponsiveDialogContent>
