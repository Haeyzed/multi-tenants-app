...
type AttributesBulkActionsProps<TData> = {
  table: Table<TData>
}

export function AttributesBulkActions<TData>({
  table,
}: AttributesBulkActionsProps<TData>) {
  const { setOpen, setExportSelection, setDeleteManySelection } = useAttributes()
  ...
  const handleExport = () => {
    const ids = selectedRows.map((row) => (row.original as Attribute).id)
    setExportSelection({
      ids,
